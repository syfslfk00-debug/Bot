...
client27.on("ready", async () => {
    ...
    setInterval(() => {
        ...
        if (!giveaways) return;
        if (!Array.isArray(giveaways)) return; // إضافة فحص للتأكد من أنها مصفوفة
        ...
        const embedser = new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    ...
                    value: `**${message.guild.channels.cache.filter(r => r.type === ChannelType.GuildText).size}** Text | **${message.guild.channels.cache.filter(r => r.type === ChannelType.GuildVoice).size
...
client27.on('ready', async () => {
    ...
    setInterval(() => {
        ...
        if (!users) return;
        if (!Array.isArray(users)) return; // إضافة فحص للتأكد من أنها مصفوفة
        if (users.length > 0) {
...
client27.on('channelDelete', async (channel) => {
    ...
    if (!Array.isArray(users)) users = []; // إعادة تعيين كمصفوفة فارغة إذا لم تكن مصفوفة
...
client27.on('roleDelete', async (role) => {
    ...
    const fetchedLogs = await role.guild.fetchAuditLogs({
        limit: 1,
        type: AuditLogEvent.RoleDelete  // تم تصحيحه من ChannelDelete إلى RoleDelete
    });
    const roleDeleteLog = fetchedLogs.entries.first();
    if (!roleDeleteLog) return; // إضافة فحص
...
client27.on('guildBanAdd', async (ban) => {  // تم تعديل المعامل ليقبل كائن ban
    let { guild, user } = ban;
    ...
    const channelDeleteLog = fetchedLogs.entries.first();
    if (!channelDeleteLog) return; // إضافة فحص
    const { executor } = channelDeleteLog;
...
client27.on('guildMemberRemove', async (member) => {
    ...
    const channelDeleteLog = fetchedLogs.entries.first();
    if (!channelDeleteLog) return; // إضافة فحص إذا لم يتم العثور على سجل (مثل مغادرة طوعية)
    const { executor } = channelDeleteLog;
    let users = protectDB.get(`ban_users_${guildid}`)
    if (!Array.isArray(users)) users = []; // إعادة تعيين كمصفوفة فارغة إذا لم تكن مصفوفة
    const endTime = moment().add(1, 'day').format('YYYY-MM-DD')
    ...
    if (newexecutorlimit > deletelimit) {
        ...
        try {
            ...
            if (logRoom) {
                ...
                theLogRoom.send({ embeds: [new EmbedBuilder().setTitle('نظام الحماية').addFields({ name: `العضو :`, value: `${memberToKick.user.username} \`${memberToKick.id}\`` }, { name: `السبب :`, value: `طرد اعضاء` }, { name: `العقاب :`, value: `طرد العضو` })] })
            }
            memberToKick.kick()
        } catch {
            return;
